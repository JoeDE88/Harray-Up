        ...
        <Box sx={{ width: '100%', maxWidth: '600px', textAlign: 'center', padding:6,pb:0, fontSize: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <CustomTabPanel value={value} index={0}>
                ...
                <Box sx={{display:"flex", flexDirection:"column"}}>
                       <>
                       <TextField onChange={(e)=>{setLoginEmail(e.target.value)}} sx={{ "& .MuiInputLabel-root": { 
                                   fontSize: "1.3rem"}, "& .MuiInputBase-input": { color: "primary.main" }}} id="standard-basic" label="Email" variant="standard" />
                       <TextField onChange={(e)=>{setLoginPassword(e.target.value)}} sx={{ "& .MuiInputLabel-root": { 
            ...
            <CustomTabPanel value={value} index={1}>
                ...
                <Box sx={{display:"flex", flexDirection:"column"}}>
                <EmailInput onChange={(e)=>{setSignInEmail(e.target.value)}}></EmailInput>
                <PasswordInput onChange={(e)=>{setSignInPassword(e.target.value)}}></PasswordInput>
                </Box>
                <div style={{ display: 'flex', justifyContent: 'center', marginTop: '30px' }}>
                    <Button onClick={()=>{login(signInEmail,signInPassword)}} variant='contained'  sx={{ backgroundColor: 'tertiary.main', color: 'tertiary.contrastText', padding: '5px 15px', fontSize: '1.3rem' }}>Sign Up</Button>
                </div>
            </CustomTabPanel>
        </Box>
    );
